...
CATEGORY_GUIDES = {
    ...
    "config": {
        "title": "AgentKit Configuration (.ck.json)",
        "workflow": [
...
def detect_prefix(commands_dir: Path) -> str:
    """Detect the slash-command prefix from the command directory structure."""
    ak_marketing_commands_dir = commands_dir / "ak" / "marketing"
    if ak_marketing_commands_dir.exists() and ak_marketing_commands_dir.is_dir():
        return "ak:"
    return ""


def normalize_command_token(command: str) -> str:
    """Normalize a typed slash command for exact-name matching."""
    token = command.lower().strip()
    token = re.sub(r"^/ak:[a-z0-9_-]+:", "", token)
...
def show_command(data: dict, command: str, prefix: str) -> None:
    ...
    search = normalize_command_token(command)

    found = None
    for cmds in commands.values():
        for cmd in cmds:
            # Normalize command name for comparison
            name = normalize_command_token(cmd["name"])
            if name == search:
                found = cmd
                break
        if found:
...
def show_config_guide() -> None:
    ...
    print("# AgentKit Configuration (.ck.json)")
    print()
