...
model: inherit
---
<!-- kit-specific: skill stack diverges — marketing activates aesthetic, brand-specific design skills (visual/brand/copy chain); engineer activates web-design-guidelines, react-best-practices, web-frameworks (code-implementation chain); same design persona, different tool context; see epic #7 -->
...
```bash
python3 $HOME/.claude/skills/ak-ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 $HOME/.claude/skills/ak-ui-ux-pro-max/scripts/search.py "<style-keywords>" --domain style
python3 $HOME/.claude/skills/ak-ui-ux-pro-max/scripts/search.py "<mood>" --domain typography
python3 $HOME/.claude/skills/ak-ui-ux-pro-max/scripts/search.py "<industry>" --domain color
```
...
**Screenshot Analysis with `ak:agent-browser`, Chrome MCP via `ak:use-mcp`, and `ak:ai-multimodal` skills**:
...
**Google Image Search**: use `WebSearch` and browser tooling through `ak:agent-browser` or Chrome MCP via `ak:use-mcp` to capture screenshots
