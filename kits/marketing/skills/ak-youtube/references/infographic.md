...
1. **Extract Video Data**
   ...
   - Run VidCap API:
     ...
     python ${CLAUDE_PLUGIN_ROOT}/skills/ak-youtube/scripts/vidcap.py caption "<url>" --ext vtt
     ```
...
3. **Design Infographic**
   ...
   - Inject brand context:
     ```bash
     node ${CLAUDE_PLUGIN_ROOT}/skills/ak-brand/scripts/inject-brand-context.cjs --json
     ```
   - Get embeddable design tokens (MANDATORY for standalone HTML):
     ```bash
     node ${CLAUDE_PLUGIN_ROOT}/skills/ak-design-system/scripts/embed-tokens.cjs --minimal
     ```
   - Structure:
...
- `youtube` - VidCap API for video extraction
...
<head>
  ...
  <style>
    ...
    :root {
      /* Paste output from: node ${CLAUDE_PLUGIN_ROOT}/skills/ak-design-system/scripts/embed-tokens.cjs --minimal */
    }
  </style>
  <style>
    body {
