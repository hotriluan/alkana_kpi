...
function checkAvailability() {
  ...
  result.ga4 = {
    ...
    message: !ga4Exists
      ? 'Install ak:analytics skill and configure GA4 credentials'
      : !ga4HasConfig
  ...
  result.gsc = {
    ...
    message: !gscExists
      ? 'Install ak:seo skill and configure GSC credentials'
      : !gscHasAuth
  ...
  result.stripe = {
    ...
    message: !paymentExists
      ? 'Install ak:payment-integration skill for Stripe metrics'
      : !stripeKey
...
function pullGA4Metrics(config) {
  if (!fs.existsSync(SKILL_PATHS.ga4)) {
    return { available: false, message: 'Install ak:analytics skill and configure GA4 credentials' };
  }
...
function pullGSCMetrics(config) {
  if (!fs.existsSync(SKILL_PATHS.gsc)) {
    return { available: false, message: 'Install ak:seo skill and configure GSC credentials' };
  }
