...
8. Suggest: `/ak:play next {slug}` to continue.
...
6. Ask user via `ask_user capability`: "What needs to change?" Store feedback.
7. Update manifest with rejection feedback.
8. Report: `Rejected "{step}". Step returned to in-progress for revision. Run /ak:play next {slug} to re-execute.`
