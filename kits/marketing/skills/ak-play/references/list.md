...
5. **Suggest Actions**
   - If active playbooks exist: `Run /ak:play next <name> to continue.`
   - If no playbooks: `Run /ak:play create to start a new playbook.`

## Error Handling

- No `data/playbooks/` directory → report no playbooks, suggest `/ak:play create`.
- Corrupted manifest → skip that playbook, warn user, suggest `/ak:play reset`.
