...
Real browser testing using Chrome MCP through `ak:use-mcp` or `ak:agent-browser` for accurate SEO analysis.
...
```bash
Configure Chrome MCP for `ak:use-mcp` or start `ak:agent-browser`
```
...
OUT=".claude/browser-seo-audits/$SESSION"
mkdir -p "$OUT"

# Performance, network, and screenshots:
# use Chrome MCP through ak:use-mcp, ak:agent-browser, Lighthouse, or project-native browser tests.
