...
1. **Fetch Page Data**
   - Use `web_search capability` tool to retrieve page content
   - If local file, use read_file capability

2. **ReviewWeb API Analysis** (if `$REVIEWWEB_API_KEY` available)
   - Reference: `${CLAUDE_PLUGIN_ROOT}/skills/ak-seo/references/reviewweb-api.md`
...
4. **Generate Report**
   - Merge web_search capability + ReviewWeb API findings
   - Score each element
...
## Skills Used
- `seo` - Audit frameworks + ReviewWeb API reference
- `assets-organizing` - Standardized output paths

## API Reference
- ReviewWeb API: `${CLAUDE_PLUGIN_ROOT}/skills/ak-seo/references/reviewweb-api.md`
...
## Data Sources
- web_search capability analysis: {yes/no}
