...
1. **Search Strategy**:
   ...
   - If `gemini` bash command is not available, fallback to the `web_search` capability.
   - Run multiple `gemini` bash commands or the `web_search` capability in parallel to search for relevant information.
