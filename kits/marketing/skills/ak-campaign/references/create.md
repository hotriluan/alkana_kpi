...
Use `ask_user capability` to gather campaign essentials:
...
Read `docs/brand-guidelines.md` for brand context. Activate `brand` skill.
...
Present options via `ask_user capability` for user selection.
...
Activate `campaign` skill → `references/launch-checklist.md`
...
Activate `campaign` skill → `references/budget-allocation.md`
...
- `campaign` - Brief templates, launch checklist
