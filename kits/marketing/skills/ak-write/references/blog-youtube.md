...
- If multiple styles exist: use `ask_user capability` to let user select
...
python ${CLAUDE_PLUGIN_ROOT}/skills/ak-youtube/scripts/vidcap.py info "$URL"
python ${CLAUDE_PLUGIN_ROOT}/skills/ak-youtube/scripts/vidcap.py summary "$URL" --screenshot
python ${CLAUDE_PLUGIN_ROOT}/skills/ak-youtube/scripts/vidcap.py caption "$URL" --locale en --ext vtt
```
...
- Invoke `/ak:design` skill (social photos sub-skill) to create 2-3 illustrations:
  - Hero/featured image matching article theme
  - Section illustrations or infographics for key points
  - Quote cards for major takeaways
- Provide design skill with: article topic, key themes, target dimensions, brand context from `docs/brand-guidelines.md`
- Use video screenshots as supplementary visuals

### 5b. Illustration Verification

- Invoke `ai-multimodal` skill to verify each generated illustration for:
...
- Save verified illustrations to output images directory
