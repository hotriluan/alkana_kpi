...
- SQLite via optional Better-SQLite3 native driver
...
```
${CLAUDE_PLUGIN_ROOT}/skills/ak-marketing-dashboard/
├── app/                        # Vue 3 frontend
│   ├── src/
...
│   └── package.json
├── data/                       # Runtime SQLite database (generated locally)
│   └── marketing.db
...
**Server (${CLAUDE_PLUGIN_ROOT}/skills/ak-marketing-dashboard/server/.env):**
```env
PORT=3457
...
**Frontend (${CLAUDE_PLUGIN_ROOT}/skills/ak-marketing-dashboard/app/.env):**
```env
...
`better-sqlite3` is an optional server dependency because it is a native SQLite
driver. If npm cannot install its prebuilt binary or compile it locally, setup
should still finish; the server will fail later at the SQLite boundary with
...
### SQLite Driver Missing Or Native Build Fails

**Error**: `better-sqlite3 is not available`

**Why it happens**: the dashboard stores asset metadata in SQLite, and
`better-sqlite3` is a native driver. Windows hosts and newer Node runtimes can
miss prebuilt binaries and require local build tools.

**Solution**:
```bash
cd ${CLAUDE_PLUGIN_ROOT}/skills/ak-marketing-dashboard/server
npm install
```

If the driver still cannot load:
- Prefer Node 20 LTS for the marketing dashboard server.
- On Windows, install Visual Studio Build Tools with the C++ desktop workload.
- Rebuild the driver after installing build tools:
  ```bash
...
Part of the AgentKit Marketing toolkit.

## Related Documentation

- App setup: `app/README.md`
- Server tests: `server/package.json`
