  ...
  >
    ...
    <div class="px-3 py-4 border-b border-border flex items-center" :class="collapsed ? 'justify-center' : 'justify-between'">
      <div class="flex items-center" :class="collapsed ? '' : 'space-x-3'">
        <div class="w-8 h-8 flex-shrink-0 rounded-md bg-primary text-white grid place-items-center text-xs font-bold">AK</div>
        <h1 v-if="!collapsed" class="text-lg font-heading font-bold text-gradient whitespace-nowrap">AgentKit</h1>
      </div>
      <button
        v-if="!collapsed"
        @click="toggleSidebar"
