...
or updated. For full doc operations, use `/ak:docs init`, `/ak:docs update`, or
`/ak:docs summarize`.
...
- `/ak:cook` and `/ak:fix`: invoke docs update during finalize only when docs
  are warranted by the criteria above.
- `the engineer plan skill`: read existing docs before creating architecture or phase plans.
- `/ak:preview`: save generated visuals under the active plan's `visuals/`
  folder when possible, then link them from docs only if they remain useful.
- `/ak:tech-graph`: use for publish-grade architecture diagrams before adding
  diagrams to docs.
