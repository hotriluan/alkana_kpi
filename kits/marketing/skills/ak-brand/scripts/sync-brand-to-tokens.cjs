...
const GENERATE_TOKENS_SCRIPT = '.claude/skills/ak-design-system/scripts/generate-tokens.cjs';
...
function updateDesignTokens(tokens, colors) {
  // Update brand name
  const brandName = `AgentKit Marketing - ${colors.primary.name.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}`;
