...
Use `ask_user capability` to collect:
...
Run the sync script:
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/ak-brand/scripts/sync-brand-to-tokens.cjs
```
...
node ${CLAUDE_PLUGIN_ROOT}/skills/ak-brand/scripts/inject-brand-context.cjs --json | head -30
