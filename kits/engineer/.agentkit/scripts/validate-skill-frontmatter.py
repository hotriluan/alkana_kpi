...
# Exact name→category mappings for high-signal AgentKit skills (avoids falling into "other")
...
def validate_frontmatter(frontmatter: dict[str, Any], skill_path: str = "") -> list[str]:
    ...
    if user_invocable is not True:
        errors.append(f"{prefix}'user-invocable' must be true for shipped AgentKit skills")

    # True is reserved for human-in-the-loop skills whose entry point is an
    # interactive interview (e.g. ak:advise, ak:brainstorm): the model never
    # auto-invokes them; users trigger them via slash command only.
