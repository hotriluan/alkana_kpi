...
Scans SKILL.md bodies for AgentKit slash-command references, builds a directed graph,
...
# Match /ak:<skill-name> but NOT inside code fences.
CODE_FENCE_RE = re.compile(r"```[\s\S]*?```", re.MULTILINE)
SKILL_REF_RE = re.compile(r"/ak:([a-z0-9][\w-]*)")
...
def _extract_body_refs(content: str, own_name: str) -> set[str]:
    """Extract AgentKit skill references from body text, excluding code fences."""
    m = FRONTMATTER_RE.match(content)
    body = content[m.end():] if m else content
    # Remove code fences to avoid false positives
    body = CODE_FENCE_RE.sub("", body)
    refs = set(SKILL_REF_RE.findall(body))
    # The regex captures the skill base, so "plan" from "/ak:plan".
    own_base = own_name.rsplit(":", 1)[-1]
    refs.discard(own_name)
    refs.discard(own_base)
    # Also discard dir-name form of own name (e.g. "ck-plan" vs "plan")
    refs.discard(own_base.replace("ck-", ""))
    return refs
...
def _run_self_tests():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixtures = {
            "cook": '---\nname: ak:cook\ndescription: "Cook things"\ncategory: utilities\nkeywords: [impl]\n---\n# Cook\nAfter planning, run /ak:scout then /ak:code-review.\n',
            "plan": '---\nname: ak:plan\ndescription: "Plan things"\ncategory: utilities\nkeywords: [plan]\n---\n# Plan\nThis skill creates plans.\n',
            "scout": '---\nname: ak:scout\ndescription: "Scout"\ncategory: dev-tools\nkeywords: [scout]\nrequires: [ak:scout]\n---\n# Scout\nExplore code. See /ak:cook for next step.\n```\n/ak:plan should not match inside fence\n```\n',
            "orphan-skill": '---\nname: ak:orphan\ndescription: "Orphan"\ncategory: other\nkeywords: []\n---\n# Orphan\nNo refs here.\n',
            "broken-ref": '---\nname: ak:broken\ndescription: "Broken"\ncategory: other\nkeywords: []\n---\n# Broken\nSee /ak:nonexistent for help.\nAlso /ak:cook is good.\n',
        }
        ...
        print("Self-tests:")
        # T1: Detects /ak:scout in cook's body
        _assert("scout" in graph["edges"].get("cook", []), "T1: Detects /ak:scout ref in body")
        # T2: cook→code-review edge exists
        _assert("code-review" in skills["cook"]["body_refs"] or "code-review" in graph["edges"].get("cook", []),
                "T2: Detects /ak:code-review ref in body")
        ...
        _assert(len(broken_refs) > 0, "T6: Broken reference /ak:nonexistent detected")
        ...
        # T8: Code fence exclusion (scout has /ak:plan in code fence, should NOT count)
