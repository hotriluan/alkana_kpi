...
try {
  ...
  try {
    ...
    if (planPath) {
      ...
      console.log(`Optional implementation command after user approval: /ak:cook ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Optional implementation command after user approval: /ak:cook {full-absolute-path-to-plan.md}');
    }
