...
Track investigation pipelines via Claude Native Tasks (manage_plan capability, manage_plan capability, manage_plan capability). Hydration pattern for multi-step investigations with dependency chains and parallel evidence collection. **Fallback:** Task tools are CLI-only — if unavailable (VSCode extension), use `manage_plan capability` for tracking. Debug workflow remains fully functional.
