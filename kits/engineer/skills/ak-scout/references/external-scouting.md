...
2. **No** - Fall back to Explore subagents (`internal-scouting.md`) only when delegation is explicitly requested/permitted

## Parallel External Commands

Prefer direct `run_shell` commands for Gemini/OpenCode probes. Use delegated
...
Use `read_file` directly or `run_shell` with `cat` for trusted paths.

**Large file (>500 lines) - use sed for ranges:**
Use `run_shell` with `sed -n 'start,endp' large-file.ts`.
...
Split reads across workers only when delegation is explicitly requested/permitted.
