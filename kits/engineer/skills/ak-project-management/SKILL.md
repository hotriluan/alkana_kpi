...
`manage_plan capability`, `manage_plan capability`, `manage_plan capability`, `manage_plan capability` are **CLI-only** — disabled in VSCode extension (`isTTY` check).

| Environment | Task Tools | Fallback |
|-------------|-----------|----------|
| CLI terminal | Available | — |
| VSCode extension | **Disabled** | `manage_plan capability` |

**Fallback behavior:** If Task tools error, use `manage_plan capability` for progress tracking. Plan file sync-back (checkbox updates, YAML frontmatter) works identically without Task tools. Core PM workflow remains functional.
...
Use `manage_plan capability`, `manage_plan capability`, `manage_plan capability`, `manage_plan capability` to manage session-scoped tasks (CLI only; see Tool Availability above).
...
- **Hydrate:** Read plan `[ ]` items → `manage_plan capability` per unchecked item
- **Work:** `manage_plan capability` tracks progress in real-time
...
1. `manage_plan capability()` — check existing tasks first
2. If empty: hydrate from plan files (unchecked items)
3. During work: `manage_plan capability` as tasks progress
...
2. Reconcile every `manage_plan capability(status: "completed")` item to phase metadata (`phase` / `phaseFile`).
