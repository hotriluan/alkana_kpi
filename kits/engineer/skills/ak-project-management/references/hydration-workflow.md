...
Task tools (`manage_plan create operation`/`manage_plan update operation`/`manage_plan read operation`/`manage_plan list operation`) are **CLI-only** — disabled in VSCode extension. If unavailable, use `manage_plan capability` for progress tracking. The hydration pattern still works: plan files remain source of truth, sync-back updates checkboxes regardless of Task tool availability.

## Session Start: Hydration

1. Read plan files: `plan.md` + `phase-XX-*.md`
2. Identify unchecked `[ ]` items = remaining work
3. `manage_plan create operation` per unchecked item with metadata (phase, priority, effort, planDir, phaseFile) — or `manage_plan capability` if Task tools unavailable
4. Set up `addBlockedBy` dependency chains between phases (skip if using manage_plan capability fallback)
5. Already-checked `[x]` items = done, skip

**Check first:** `manage_plan list operation()` — if tasks already exist (same session), skip re-creation. If manage_plan list operation errors, proceed with manage_plan capability.
...
1. Collect completed tasks (`manage_plan update operation(status: "completed")`) with metadata (`phase`, `phaseFile`, `planDir`).
...
1. `manage_plan list operation()` → empty (tasks died with old session)
