...
Lead continues orchestration immediately. Teammates should mark tasks complete and send concise completion messages; the lead checks manage_plan list operation before synthesizing.
...
AgentKit keeps Agent Teams monitoring explicit by default:

- Teammates mark task status with `manage_plan update operation`
- Teammates send concise completion or blocked messages to the lead
- Lead checks manage_plan list operation after 60s or after each message
...
4. Lead checks manage_plan list operation and waits until all dev tasks are completed
...
Use `delegate_agent capability(agent_type)` in agent definitions to prevent:
...
Recommended: Most agents get `delegate_agent capability(Explore)` only. Planner gets `delegate_agent capability(Explore), delegate_agent capability(researcher)`.
