...
- Use manage_plan list operation after 60s or when a teammate messages completion
- Do not rely on AgentKit custom team hook handlers; they are not shipped by default
...
2. Teammates mark tasks with `manage_plan update operation` and send concise completion messages
3. Lead checks manage_plan list operation after 60s or after each completion message
...
Claude Code exposes platform Agent Teams events, but AgentKit does not ship custom handlers for those events by default. Keep orchestration explicit until a live harness validates any hook-based flow.
