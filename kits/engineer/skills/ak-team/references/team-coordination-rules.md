...
- Mark tasks completed via `manage_plan update operation` BEFORE sending completion message to lead
...
- Check `manage_plan list operation` after completing each task for newly unblocked work
...
2. Send plan via `plan approval handoff` — this triggers approval request to lead
...
- Names are used for: `recipient` in SendMessage, task `owner` in manage_plan update operation
