...
|---|---|---|
| **Tool** | `Agent` (formerly `Task`) | `Agent` + `TeamCreate`/`manage_plan create operation`/`SendMessage` | <!-- capability-lint-allow: Agent Teams native API reference -->
...
The `Agent` tool (formerly `Task`, renamed v2.1.63) spawns teammates: <!-- capability-lint-allow: Agent Teams native API history -->
...
| `blocks` | Task IDs this task blocks (read via manage_plan read operation) |
| `blockedBy` | Task IDs that must complete first (read via manage_plan read operation) |
| `addBlocks` | Set blocking relations (write via manage_plan update operation) |
| `addBlockedBy` | Set dependency relations (write via manage_plan update operation) |
...
These are Claude Code platform events. AgentKit does not ship custom handlers for them by default; team workflows should monitor manage_plan list operation and teammate messages unless a project deliberately adds and validates its own handlers.

### TaskCompleted

Fires when teammate calls `manage_plan update operation` with `status: "completed"`.
...
## delegate_agent capability(agent_type) Restrictions

Limit which sub-agents an agent can spawn:

```yaml
tools: Read, Grep, Bash, delegate_agent capability(Explore)
```
