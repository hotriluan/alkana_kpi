...
# Agent Teams - AgentKit-native Orchestration Engine
...
**Requires:** CLI terminal -- task-management capability and `TeamCreate`/`TeamDelete` are **disabled in VSCode extension** (`isTTY` check). Agent Teams CANNOT run in VSCode.
...
Agent(
  ...
  prompt: "full instructions + AgentKit Context Block",
...
**Note:** `Task` was renamed to `Agent` in v2.1.63. Both names work; prefer `Agent` for new code. <!-- capability-lint-allow: Agent Teams native API history -->
...
| manage_plan create operation | Create work item | `subject`, `description`, `priority`, dependencies |
| manage_plan update operation | Claim/complete task | `taskId`, `status`, `owner`, `metadata` |
| manage_plan read operation | Full task details | `taskId` |
| manage_plan list operation | All tasks (minimal fields) | *none* |
...
- Team rules: MUST read `references/team-coordination-rules.md` before work
...
3. **CREATE** N work items through the manage_plan capability -- one per angle:
...
4. **SPAWN** teammates x N via `Agent` tool:
   ...
   - `name: "researcher-{N}"`
   - Prompt: task description + AgentKit Context Block

5. **MONITOR** via manage_plan list operation + direct messages:
   - Check task status after 60s or when a teammate messages completion
   - Do not rely on AgentKit custom TaskCompleted/TeammateIdle handlers; they are not shipped by default
...
11. **JOURNAL**: Run `/ak:journal` to write a concise technical journal entry upon completion
...
3. **CREATE** (N + 1) work items through the manage_plan capability -- N dev tasks + 1 tester task:
...
4. **SPAWN** developer teammates x N via `Agent` tool:
   ...
   - `name: "dev-{N}"`
   - Prompt: task description + plan context + AgentKit Context Block
...
5. **MONITOR** dev completion via manage_plan list operation + direct messages:
   - Check task status after 60s or when a teammate messages completion
...
7. **DOCS SYNC EVAL** (MANDATORY for cook -- from /ak:cook finalize):
   ```
...
3. **CREATE** N work items through the manage_plan capability -- one per focus:
...
10. **JOURNAL**: Run `/ak:journal` to write a concise technical journal entry upon completion
...
3. **CREATE** N work items through the manage_plan capability -- one per hypothesis:
...
5. **MONITOR** via manage_plan list operation + messages. Debuggers should message each other -- let them converge.
   - Check task status after 60s or when a debugger reports findings
...
4. **Reassign**: update stuck task through the manage_plan capability to unblock dependents
...
See `references/team-coordination-rules.md` for teammate behavior rules. Keep these rules skill-local so normal sessions do not spend startup context on Agent Teams.
