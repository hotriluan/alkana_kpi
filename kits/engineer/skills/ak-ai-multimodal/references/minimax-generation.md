...
```bash
# Image
npx -y -p @mrgoonie/multix@0.2.0 multix minimax generate --prompt "A cyberpunk city" --model image-01 --aspect-ratio 16:9 --output city.png

# Video
npx -y -p @mrgoonie/multix@0.2.0 multix minimax generate-video --prompt "A dancer" --model MiniMax-Hailuo-2.3 --duration 6 --output dancer.mp4

# Speech
npx -y -p @mrgoonie/multix@0.2.0 multix minimax generate-speech --text "Hello world" --model speech-2.8-hd --voice English_Warm_Bestie --emotion happy --output hello.mp3

# Music
npx -y -p @mrgoonie/multix@0.2.0 multix minimax generate-music --lyrics "La la la\nOh yeah" --prompt "upbeat pop" --model music-2.5 --output song.mp3
```
