...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.claude/.ck.json` (or `~/.claude/.ck.json`) for `skills.research.useGemini` (default: `false`). If `false` or absent, skip Gemini and use web_search capability directly.
   - **Gemini Model**: Read from `.claude/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
   - If `useGemini` is `true`: first validate Gemini CLI works: `command -v gemini >/dev/null 2>&1 && cd /tmp && timeout 15 gemini -y -m <gemini.model> --prompt "ping" >/dev/null 2>&1`. If validation fails or times out, fall back to web_search capability and warn: "Gemini CLI unavailable or auth failed, using web_search capability."
   - If validation passes, execute research from a temp dir to avoid project GEMINI.md interception (note: global `~/.gemini/GEMINI.md` still loads): `cd /tmp && timeout 180 gemini -y -m <gemini.model> --prompt "...your search prompt..." 2>&1`. Check exit code — if non-zero or output contains `GaxiosError`, `RESOURCE_EXHAUSTED`, `MODEL_CAPACITY_EXHAUSTED`, `PERMISSION_DENIED`, or `UNAUTHENTICATED`, fall back to web_search capability for that query and warn: "Gemini CLI failed, falling back to web_search capability." Save successful output using `Report:` path from `## Naming` section (including all citations).
   - If `useGemini` is disabled or `gemini` bash command is not available, use `web_search capability` tool.
   - Run multiple `gemini` bash commands or `web_search capability` tools in parallel to search for relevant information.
